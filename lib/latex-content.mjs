...
export const SUPPORTED_FAMILIES = ['resumeSubheading', 'tabularx-itemize', 'xelatex-custom-macros'];

export const UNSUPPORTED_HINT =
  'Unsupported LaTeX CV layout. v1 supports \\resumeSubheading + \\resumeItem macros, ' +
  'tabularx + itemize without resume macros, or XeLaTeX custom macros (\\role, \\subline). Use /career-ops latex (cv.md → career-ops template) instead.';
...
/**
 * @param {string} tex
 * @returns {'resumeSubheading'|'tabularx-itemize'|'xelatex-custom-macros'|null}
 */
export function detectFamily(tex) {
  if (typeof tex !== 'string' || !tex.trim()) return null;
  if (/\\newcommand\s*\{\\role\}/.test(tex) && /\\newcommand\s*\{\\subline\}/.test(tex)) {
    return 'xelatex-custom-macros';
  }
...
function extractItemizeItems(tex) {
  ...
  while ((match = itemRe.exec(body)) !== null) {
    if (!isCommentedAt(body, match.index)) {
      matches.push(match);
    }
  }
  ...
  for (let m = 0; m < matches.length; m++) {
    ...
    let i = currentMatch.index + currentMatch[0].length;
    ...
    if (m + 1 < matches.length) {
      nextIdx = matches[m + 1].index;
    }
    const endItemize = body.indexOf('\\end{itemize}', i);
    if (endItemize !== -1 && endItemize < nextIdx) {
      nextIdx = endItemize;
    }
    const endDoc = body.indexOf('\\end{document}', i);
    if (endDoc !== -1 && endDoc < nextIdx) {
      nextIdx = endDoc;
    }

    const rawText = body.slice(i, nextIdx);
    const text = rawText.trim();
    if (!text) continue;

    const trimmedRight = rawText.trimEnd();
    const endPos = i + trimmedRight.length;

    slots.push({
      id: `item-${slots.length}`,
      kind: 'item',
      text,
      span: { start: offset + i, end: offset + endPos },
    });
  }
  return slots;
}
 ...
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
function extractXelatexCustomSlots(tex) {
  const slots = [];

  // 1. Professional Summary slot
  const summarySec = tex.match(/\\section\*?\{(?:Professional\s+)?Summary\}/i);
  if (summarySec) {
    const startIdx = summarySec.index + summarySec[0].length;
    const nextSecMatch = tex.slice(startIdx).match(/\\section\*?\{/);
    let nextSecIdx = nextSecMatch ? startIdx + nextSecMatch.index : -1;
    if (nextSecIdx === -1) nextSecIdx = tex.indexOf('\\end{document}', startIdx);
    if (nextSecIdx === -1) nextSecIdx = tex.length;

    const summaryRegion = tex.slice(startIdx, nextSecIdx);
    const lines = summaryRegion.split('\n');
    let pStart = -1;
    let pEnd = -1;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lineTrim && !lineTrim.startsWith('%')) {
        if (pStart === -1) {
          pStart = currentPos + (line.length - line.trimStart().length);
        }
        pEnd = currentPos + line.trimEnd().length;
      } else if (pStart !== -1) {
        break;
      }
    ...
    if (pStart !== -1 && pEnd !== -1) {
      slots.push({
        id: 'summary-0',
        kind: 'summary',
        text: tex.slice(pStart, pEnd).trim(),
        span: { start: pStart, end: pEnd },
      });
    }
  }

  // 2. Skill lines (\textbf{Cat:} \ VALUE\par or \textbf{Cat}: VALUE\par)
  const skillRe = /\\textbf\{([^}]+):?\}(?:\s*:)?/g;
  let skillMatch;
  let skillCount = 0;
  while ((skillMatch = skillRe.exec(tex)) !== null) {
    if (isCommentedAt(tex, skillMatch.index)) continue;
    let valStart = skillMatch.index + skillMatch[0].length;
    while (valStart < tex.length) {
      const ch = tex[valStart];
      if (/[\s~]/.test(ch)) {
        valStart++;
      } else if (ch === '\\' && tex[valStart + 1] === ' ') {
        valStart += 2;
      } else {
        break;
      }
    ...
    let valEnd = tex.length;
    const parIdx = tex.indexOf('\\par', valStart);
    const breakIdx = tex.indexOf('\\\\', valStart);
    const dblNlIdx = tex.indexOf('\n\n', valStart);
    const nlIdx = tex.indexOf('\n', valStart);

    // Prefer explicit LaTeX line terminators (\par or \\) over single newline
    if (parIdx !== -1 && (parIdx <= nlIdx || dblNlIdx === -1 || parIdx < dblNlIdx)) {
      valEnd = parIdx;
    } else if (breakIdx !== -1 && (breakIdx <= nlIdx || dblNlIdx === -1 || breakIdx < dblNlIdx)) {
      valEnd = breakIdx;
    } else {
      const candidates = [dblNlIdx, nlIdx].filter(i => i !== -1);
      if (candidates.length > 0) valEnd = Math.min(...candidates);
    }

    const valText = tex.slice(valStart, valEnd).trim();
    if (!valText) continue;

    const trimmedRight = tex.slice(valStart, valEnd).trimEnd();
    const spanEnd = valStart + trimmedRight.length;

    slots.push({
      id: `skill-${skillCount++}`,
      kind: 'skill',
      text: valText,
      span: { start: valStart, end: spanEnd },
    });
  }

  // 3. Itemize bullets
  const itemSlots = extractItemizeItems(tex);
  itemSlots.forEach((s, idx) => {
    slots.push({
      id: `bullet-${idx}`,
      kind: 'bullet',
      text: s.text,
...
/**
 * @param {string} tex
 * @param {'resumeSubheading'|'tabularx-itemize'|'xelatex-custom-macros'} family
 * @returns {Array<{id: string, kind: string, text: string, span: {start: number, end: number}}>}
 */
export function extractSlots(tex, family) {
  if (family === 'xelatex-custom-macros') {
    return extractXelatexCustomSlots(tex);
  }
...
export function applyPatches(tex, patches, slots, { escape = true } = {}) {
  ...
  const patchBySlotId = new Map();
  for (const p of patches) {
    if (p && p.id) patchBySlotId.set(p.id, p);
  }
  const ordered = Array.from(patchBySlotId.values())
    .map(p => {
      const slot = slotById.get(p.id);
      if (!slot) return null;
      const doEscape = p.escape !== undefined ? !!p.escape : escape;
      return { slot, text: p.text, escape: doEscape };
    })
    .filter(Boolean)
    .sort((a, b) => b.slot.span.start - a.slot.span.start);

  let out = tex;
  for (const { slot, text, escape: doEscape } of ordered) {
    const replacement = doEscape ? escapeLatex(text) : text;
