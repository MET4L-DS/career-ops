...
export function detectFamily(tex) {
  ...
  if (/\\newcommand\{\\role\}/.test(tex) && /\\newcommand\{\\subline\}/.test(tex)) {
...
function extractXelatexCustomSlots(tex) {
  ...
  if (summarySec) {
    ...
    let nextSecIdx = tex.indexOf('\\section*{', startIdx);
  ...
  // 2. Skill lines (\textbf{Cat:} \ VALUE\par or line end)
  const skillRe = /\\textbf\{([^}]+):\}/g;
  let skillMatch;
  let skillCount = 0;
  while ((skillMatch = skillRe.exec(tex)) !== null) {
    ...
    while (valStart < tex.length && /[\s\\]/.test(tex[valStart]) && !tex.startsWith('\\par', valStart)) valStart++;
    
    let valEnd = tex.length;
    const parIdx = tex.indexOf('\\par', valStart);
    const breakIdx = tex.indexOf('\\\\', valStart);
    const nlIdx = tex.indexOf('\n', valStart);
    const candidates = [parIdx, breakIdx, nlIdx].filter(i => i !== -1);
